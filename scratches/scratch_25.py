...
def pokerEvent(id, type, *args):
    ...
    if type == "all-in":
                    ...
                    if frame['type'] == 'left_over':
                        if frame['serial'] == mySerial:
                            message = _("You receive %(amount)s odd chips") % {
                                'amount': PokerChips.tostring(frame['chips_left'])}
                        else:
                            message = _("Player %(number)s receives %(amount)s odd chips") % {'number': frame['serial'],
                                                                                              'amount': PokerChips.tostring(
                                                                                                  frame['chips_left'])}
                    elif frame['type'] == 'uncalled':
                        message = _("returning uncalled bet %(amount)s to %(name)s") % {
                            'amount': PokerChips.tostring(frame['uncalled']), 'name': serial2name(frame['serial'])}
                            ...
                            for side in ('hi', 'low'):
                                ...
                                if show:
                                    ...
                                    if serial == mySerial:
                                        ...
                                        info(_("Player %(number)d shows %(value)s") % {'number': serial, 'value': value,
                                                                                       'side': side})
                                else:
                                    if serial == mySerial:
                        ...
                        for (serial, share) in frame['serial2share'].iteritems():
                            if serial == mySerial:
                                ...
                                info(_("Player %(number)d receives %(amount)s") % {'number': serial,
                                                                                   'amount': PokerChips.tostring(
                                                                                       share)})
        ...
        info(_("Hand #%(hand_serial)d, %(variant)s, %(betting_structure)s") % {'hand_serial': hand_serial,
                                                                               'variant': _(variant),
                                                                               'betting_structure': _(
                                                                                   betting_structure)})
        if len(set(serial2chips.values())) == 1:
            ...
            for serial in player_list:
                if serial == mySerial:
                    ...
                    info(_("Player %(serial)d has %(chips)s in chips") % {'serial': serial,
                                                                          'chips': PokerChips.tostring(
                                                                              serial2chips[serial])})
